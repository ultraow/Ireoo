...
<ol class="account">
    <h1>新增收货地址</h1>
    <span class="h2">所有项目必填</span>

    <?php if(isset($_GET['m']) and $_GET['m'] == 'edit') {
        $s = array(
            'table' => 'address',
        ...
        $r = $mysql->row($s);
        ?>
        <form action="?s=person&i=address&m=edit&id=<?php echo $_GET['id']; ?>&token=<?php echo md5(rand(0, 100000000)); ?>" method="post">
            <li>
                <label>详细地址：</label>
            ...
            <li>
                <label>邮编：</label>
                <input name="port" value="<?php echo $r['port']; ?>" />
            </li>
            <li>
                <label>联系人：</label>
    ...
    <?php }else{ ?>
        <form action="?s=person&i=address&token=<?php echo md5(rand(0, 100000000)); ?>" method="post">
            <li>
                <label>详细地址：</label>
        ...
        ?>
            <tr>
                <td>
                    <?php echo $v['address']; ?>
                    <?php if($v['def'] == 0) { ?>
                    <a href="?s=person&i=address&m=default&id=<?php echo $v['id']; ?>&token=<?php echo md5(rand(0, 100000000)); ?>">设为默认</a>
                    <?php }else{ ?>
                    <span style="color: green;">［默认］</span>
                    <?php } ?>
                </td>
                ...
                <td>
                    <a href="?s=person&i=address&m=edit&id=<?php echo $v['id']; ?>">修改</a>
                    <a href="?s=person&i=address&m=del&id=<?php echo $v['id']; ?>&token=<?php echo md5(rand(0, 100000000)); ?>">删除</a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
