<style type="text/css">
    ...
    div.index ul li{padding: 10px; overflow: auto; border-top: 1px RGB(242, 242, 242) dashed; position: relative;}
    div.index ul li.first{border-top: none;}
    div.index ul li span.img{width: 60px; position: absolute; top: 15px; left: 0;}
    div.index ul li span.img img{width: 60px; height: 60px;}
    div.index ul li div{font-weight: normal; font-family: Arial, Helvetica, sans-serif;}

    div.index ul li div div.img{padding: 0; width: 100%; max-height: 250px; overflow: hidden; margin-bottom: 10px;}
    div.index ul li div div.img img{float: none; width: 100%; height: auto;}

    div.index ul li div h1{font-size: 12px; margin-bottom: 10px;}
    div.index ul li div h1 a.name{color: #000; text-decoration: none; font-size: 14px;}
        ...
        <?php $say = $store->getSay($mysql); if(is_array($say)){ foreach($say as $k => $v) { ?>
            <li<?php if($k == 0){echo ' class="first"';} ?>>
                ...
                <span>
                    ...
                    <em>
                        <a class="timer" rel="<?php echo date('Y年m月d日，H时i分', $v['timer']); ?>"><?php new timer($v['timer']); ?></a> 查看 <a href="http://t.ireoo.com/<?php echo $v['id']; ?>">详细信息</a>
                    </em>
                </div>
                <br />
            </li>
